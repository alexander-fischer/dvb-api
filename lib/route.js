...
var route = function route(origin, destination, time, forArrival, numberOfResults, callback) {
    ...
    function createResponse(error, response, body) {
        ...
        if (!error && response.statusCode == 200) {
            ...
            var hasErrorMessage = json['Trias']['ServiceDelivery']['DeliveryPayload']['TripResponse']['ErrorMessage'] != undefined

            var outputJson
            if (hasErrorMessage) {
                console.log('Error occured')
                response.statusCode = 500
                outputJson = 'Error occured'
            } else {
                outputJson = processResponseJson(json)
            }
...
function processResponseJson(json) {
    ...
    for (var i = 0; i < tripResults.length; i++) {
        var startPoint, stopPoint, nrOfInterchanges
        var routes = []
        ...
        if (hasInterchanges) {
            ...
            for (var j = 0; j < tripLeg.length; j++) {
                ...
                nrOfInterchanges = routes.length - 1
            }
            // Check route for single trip
        } else {
            var timedLeg = tripLeg["TimedLeg"]
            var continousLeg = tripLeg["ContinuousLeg"]
            let sVehicleType, line, lineDirection
        ...
        var newJson = {
            ...
            stopTime: endTime,
            duration: duration,
            nrOfInterchanges: nrOfInterchanges,
            route: routes
        }
